...
void malloc_run_state(RunState* s, Config* p) {
    ...
    s->x = heap_caps_calloc_psram(p->dim, sizeof(float));
    s->xb = heap_caps_calloc_psram(p->dim, sizeof(float));
    s->xb2 = heap_caps_calloc_psram(p->dim, sizeof(float));
    s->hb = heap_caps_calloc_psram(p->hidden_dim, sizeof(float));
    s->hb2 = heap_caps_calloc_psram(p->hidden_dim, sizeof(float));
    s->q = heap_caps_calloc_psram(p->dim, sizeof(float));
    s->key_cache = heap_caps_calloc_psram(p->n_layers * p->seq_len * kv_dim, sizeof(float));
    s->value_cache = heap_caps_calloc_psram(p->n_layers * p->seq_len * kv_dim, sizeof(float));
    s->att = heap_caps_calloc_psram(p->n_heads * p->seq_len, sizeof(float));
    s->logits = heap_caps_calloc_psram(p->vocab_size, sizeof(float));
...
void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights,
    ...
    // ESP32 port: the weights live in a RAW flash partition, memory-mapped.
    // This is the whole trick — the MMU serves weight reads straight from
    // flash, so a model far larger than RAM still works. It only works on a
    // raw partition; a file inside LittleFS cannot be mapped.
    const esp_partition_t* part = esp_partition_find_first(
        ESP_PARTITION_TYPE_DATA, (esp_partition_subtype_t)0x40, "model");
    if (!part) { ESP_LOGE("tinylm", "no 'model' partition"); exit(1); }
    const void* base = NULL;
    static esp_partition_mmap_handle_t handle;
    if (esp_partition_mmap(part, 0, part->size, ESP_PARTITION_MMAP_DATA,
                           &base, &handle) != ESP_OK) {
        ESP_LOGE("tinylm", "esp_partition_mmap failed"); exit(1);
    }
...
void free_transformer(Transformer* t) {
    // ESP32 port: the weights are an esp_partition mmap that lives for the
    // life of the program, so there is nothing to unmap here.
...
void build_tokenizer(Tokenizer* t, char* tokenizer_path, int vocab_size) {
    // ESP32 port: read the tokenizer out of the embedded array (tok512.h).
...
/* CLI entry point removed for the ESP32 port — see main.cpp */

// ESP32 port addition: copy the whole weight blob out of memory-mapped flash
// into PSRAM and re-point the weight pointers at it. Every token reads all the
// weights, and the working set is far larger than the MMU cache, so this
